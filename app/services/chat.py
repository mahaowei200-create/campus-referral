...
class ChatService:
    ...
    async def stream_chat(self, user: UserAccount, request: ChatRequest):
        ...
        async for token in self.ai.stream(outcome.response_messages):
            ...
            yield sse("token", ChatStreamEvent(type="token", sessionId=outcome.session.public_id, content=token).model_dump())#把拿到的token逐渐渲染到前端
        if assistant:
            self.agent_harness.save_assistant_message(user, outcome.session, "".join(assistant))   #把整个AI回复的字符串存入数据库
        try:
        ...
        yield sse("done", ChatStreamEvent(type="done", sessionId=outcome.session.public_id).model_dump())#通知前端本轮流式输出结束

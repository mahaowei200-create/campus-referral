...
class AgentContext:
    ...
    original_input: str
    model_input: str
    memory_loaded: bool = False   #加载记忆
    intent_routed: bool = False   #识别意图
    knowledge_handled: bool = False  #检索知识
    risk_assessed: bool = False    #评估风险
    response_planned: bool = False  #回复准备
    finished: bool = False          #执行结束
    memory_brief: str = "无相关历史记忆。"   #历史记忆摘要
    intent: IntentType | None = None    #意图， chat，consult，risk
...
class AgentRuntimeService:
    ...
    def memory_agent(self, step: int, context: AgentContext) -> bool:
        if context.memory_loaded:
            return False
        history = self.memory.load_recent(context.session.public_id)   #从redis中读取短期记忆
        source = "redis"  
        if not history:       #如果redis中为空，从mysql里边拿记忆
            rows = (
            ...
            if history:      #如果从数据库读到了数据
                self.memory.replace(context.session.public_id, history)  #把数据写回redis缓存
                source = "mysql_seeded"
                #对原始历史做压缩，过长历史做精简
                ## compact_history_for_prompt：工具函数，输出压缩后的历史消息 + 兜底摘要（LLM摘要失败时用）
        compacted_history, deterministic_brief = compact_history_for_prompt(history, self.settings, context.model_input) 
        context.model_history = self._bounded_model_history(  #把压缩后的历史 + 当前用户最新输入，合并，再做一次长度上限裁剪
            [*compacted_history, AiMessage(role="user", content=context.model_input)]
        )
        context.memory_brief = self._summarize_memory(history, context.model_input, deterministic_brief)  #调用LLM生成简短记忆摘要，放到system prompt给后面Agent看
    ...
    def supervisor_agent(self, step: int, context: AgentContext) -> bool:    #监管Agent 判断是chat，consult，risk
    ...
    def knowledge_agent(self, step: int, context: AgentContext) -> bool:  #它先调用模型把用户问题改写成适合检索的查询词
        ...
        retrieved = self.knowledge.retrieve(query, self.settings.knowledge_top_k)   #拿到知识库内容，保存到 retrieved_knowledge，供 CounselorAgent 构造系统提示词
    ...
    def risk_guardian_agent(self, step: int, context: AgentContext) -> bool:   #调用心理评估服务，情绪类型，情绪分数，风险等级，置信度，摘要
    ...
    def companion_agent(self, step: int, context: AgentContext) -> bool:   #专门处理普通 CHAT
    ...
    def counselor_agent(self, step: int, context: AgentContext) -> bool:   #整理四类信息，心理意图和风险等级+ KnowledgeAgent 检索内容+ SkillLibrary 提供的回复指引+ MemoryAgent 的记忆和历史消息
        ...
        context.response_messages = [
            PromptTemplates.answer_system_prompt(    #然后构造 response_messages，交给大模型流式生成最终回答
                context.intent or IntentType.CONSULT,
                context.risk_level,
                knowledge_context,
    ...
    def _bounded_model_history(self, history: list[AiMessage]) -> list[AiMessage]:   #压缩用户最新的历史

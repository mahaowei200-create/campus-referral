...
def test_compaction_keeps_recent_messages_and_adds_internal_summary():
    history = [
        AiMessage(
            role="user" if i % 2 == 0 else "assistant",
            content=f"第{i}条消息 13800138000",
        )
        for i in range(10)
    ]
    ...
    assert compacted[-1].content.startswith("第9条消息")
    assert "13800138000" not in brief
    assert "[已脱敏]" in brief
